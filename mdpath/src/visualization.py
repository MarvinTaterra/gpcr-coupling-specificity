...
class MDPathVisualize:
    ...
    def apply_backtracking(original_dict: dict, translation_dict: dict) -> dict:
        ...
        Only includes pathways where ALL residues have valid coordinates.
            ...
            for inner_list in lists_of_lists:
                ...
                for item in inner_list:
                    if item in translation_dict:
                        new_inner_list.append(translation_dict[item])
                    else:
                        missing_residues.add(item)
                        valid_pathway = False
                        # Don't break - collect all missing residues for reporting
                
                if valid_pathway:
                    updated_dict[key].append(new_inner_list)
                else:
                    skipped_pathways += 1

        if missing_residues:
            sorted_missing = sorted(missing_residues)
            if len(sorted_missing) > 10:
                display_missing = sorted_missing[:10]
                print(f"Warning: {len(missing_residues)} residues not found in coordinate dictionary: {display_missing}... (and {len(sorted_missing) - 10} more)")
            else:
                print(f"Warning: {len(missing_residues)} residues not found in coordinate dictionary: {sorted_missing}")
            print(f"  Skipped {skipped_pathways} of {total_pathways} pathways due to missing coordinates.")
    ...
    def _extract_coordinates(item):
        """Helper function to extract coordinates from various data formats.
        
        Args:
            item: Can be a list of coordinates [[x,y,z]], a coordinate [x,y,z], or other.
            
        Returns:
            list or None: The coordinates [x,y,z] if valid, None otherwise.
        """
        if not isinstance(item, list):
            return None
        
        # Check if item is [[x,y,z]] format (list containing a coordinate list)
        if len(item) == 1 and isinstance(item[0], list) and len(item[0]) == 3:
            return item[0]
        
        # Check if item is [x,y,z] format directly
        if len(item) == 3 and all(isinstance(x, (int, float)) for x in item):
            return item
        
        # Check if item is [[x,y,z], ...] format with multiple coordinates
        if len(item) > 0 and isinstance(item[0], list) and len(item[0]) == 3:
            return item[0]
    ...
    def precompute_path_properties(json_data: dict) -> list:
        ...
        for clusterid, cluster in json_data.items():
            ...
            for pathway_index, pathway in enumerate(cluster):
                for i in range(len(pathway) - 1):
                    ...
                    coord1 = MDPathVisualize._extract_coordinates(item1)
                    coord2 = MDPathVisualize._extract_coordinates(item2)
        ...
        if skipped_segments > 0:
            print(f"Warning: Skipped {skipped_segments} path segments due to invalid coordinate format.")
